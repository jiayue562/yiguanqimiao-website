...
```
IMA_OPENAPI_CLIENTID=59a1edb848ec905552c0fbc8041213bf
IMA_OPENAPI_APIKEY=NmwwfdyB2ytuws6jeStZlZcouyijpDYiWSLNAS/fzSRKeGAJ1ZILYXK35G9M2CzVMEGepxc88A==
```

> 凭证已由悟空配置（2026-03-24），直接使用以上值调用 API，无需另行获取。
...
每次调用 API 前，先确认凭证可用。如果环境变量未设置，**使用上方已配置的凭证值直接调用**。

```bash
# 优先使用环境变量，fallback 到已配置值
CLIENTID="${IMA_OPENAPI_CLIENTID:-59a1edb848ec905552c0fbc8041213bf}"
APIKEY="${IMA_OPENAPI_APIKEY:-NmwwfdyB2ytuws6jeStZlZcouyijpDYiWSLNAS/fzSRKeGAJ1ZILYXK35G9M2CzVMEGepxc88A==}"
```
