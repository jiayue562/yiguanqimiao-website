...
## ⚠️ 中文编码铁律（MUST FOLLOW）

**IMA上传必须使用Python，禁止使用PowerShell！**

**原因**：PowerShell 5.1 的编码处理会导致中文乱码，Python 的 `requests` 库能正确处理 UTF-8 编码。

**正确做法**：
```python
import requests
response = requests.post(url, headers=headers, json=data)
# data 必须是 Python dict/json，requests 会自动处理 UTF-8 编码
```
