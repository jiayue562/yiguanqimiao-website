...
文档结构标准:
  ...
  章节顺序:
    推荐顺序: 标题 → 基本信息 → 核心定义 → 详细内容 → 关联 → 金句 → 标签
    允许调整: 根据文档类型适当调整
...
Python包依赖:
  - 标准库: os, re, json, pathlib, datetime, collections
  - 可选包: networkx (用于高级网络分析)
```
...
# 3. 配置知识库路径
编辑 config.yaml 设置知识库路径
```

#### 手动安装
```bash
# 1. 创建项目目录
mkdir knowledge-base-validator
cd knowledge-base-validator
...
# 3. 创建配置文件
echo "knowledge_base_path: /path/to/your/knowledge-base" > config.yaml

# 4. 创建启动脚本
创建 run_validation.bat (Windows) 或 run_validation.sh (Linux/macOS)
```
...
```bash
# 基本验证
python main_validator.py --path "/path/to/knowledge-base"

# 指定输出目录
python main_validator.py --path "/path/to/knowledge-base" --output "./my-reports"

# 仅验证特定模块
python link_validator.py --path "/path/to/knowledge-base"
python structure_validator.py --path "/path/to/knowledge-base"
```
...
任务名称: 每日知识库验证
触发器: 每日 02:00
操作: 运行 main_validator.py
参数: --path "C:\KnowledgeBase" --output "C:\Reports"

# Linux cron 任务
0 2 * * * cd /opt/knowledge-base-validator && python main_validator.py --path "/data/knowledge-base"
```
...
```
验证报告/
├── 📄 knowledge_base_validation_YYYYMMDD_HHMMSS.md  # 主报告
