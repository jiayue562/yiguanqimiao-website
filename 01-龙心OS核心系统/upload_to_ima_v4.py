...
def upload_file_to_ima(file_path, kb_id, folder_id=None):
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            ...
            print(f"   🔍 API响应: {response_data[:200]}")
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            ...
            if response_code == 0:
                ...
                error_msg = result.get('msg', result.get('message', json.dumps(result, ensure_ascii=False)))
                raise Exception(f"添加到知识库失败: {error_msg}")
...
def main():
    ...
    try:
        note_id_1 = upload_file_to_ima(file1, KB_ID, FOLDER_ID)
    ...
    try:
        note_id_2 = upload_file_to_ima(file2, KB_ID, FOLDER_ID)
